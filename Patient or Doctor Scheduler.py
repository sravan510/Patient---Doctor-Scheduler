...
class Patient(object):
    ...
    def schedule_time(self,dc,sp,fn,ln,age,date):
        ...
        for i in timing:
            print i
        while(True):
            a = raw_input("Please enter required slot: ")
            if(a in date):
                date.remove(a)
                break
            else:
...
while(True):
    ...
    print "\n\n"
    for key, value in doctors.iteritems():
        print "Name: {x}  speciality: {y}".format(x=key,y=value)
        #print "Please enter the name and specialists of a doctor to choose"
    while(True):
        dc = raw_input("Enter the name of doctor you want to visit: ")
        if(dc == ""):
            print "Please enter correct name"
        else:
            break
    while(True):
        sp = raw_input("Enter the specialists: ")
        if (sp == ""):
            print "Please enter correct name"
        else:
            break
    ...
    while(True):
        ...
        if (age == ""):
            ...
            break
    while (True):
        date = raw_input("Please enter the data on which date you want appointment (Format of date : MM-DD-YEAR):")
        if (date == ""):
